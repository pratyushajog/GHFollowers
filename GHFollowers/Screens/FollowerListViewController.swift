...
//  GHFollowers
//
//  Created by Pratyusha Joginipally on 10/9/23.
//
...
class FollowerListViewController: UIViewController {
  ...
  func getFollowers(username: String, page: Int) {
    showLoadingView()
    // We can use the Result type here in the closure. Watch the "Result type refactor".
    // I did not make the changes since Result type was introduced in later iOS versions
    ...
    NetworkManager.shared.getUserInfo(for: username) { [weak self] user, error in
      ...
      PersistenceManager.updateWith(favorite: favorite, actionType: .add) { [weak self] error in
        ...
        guard let error = error else {
          self.presentGFAlertOnMainThread(title: "Success!", message: "Added the user to Favorites", buttonTitle: "Ok")
          return
        }
